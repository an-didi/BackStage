...
class AuthRule extends Model
{
    ...
    public function AuthRule($data): array
    {
        $draw = $data['draw'];
        // order参数，从那一列开始进行排序
        ...
        if (strlen($search) > 0){
            $recordsFilteredResult = AuthRule::where('title|name|id','like', '%' . $search . '%')->select()->toArray();
            $recordsFiltered = count($recordsFilteredResult);
        }else{
            // 没有搜索条件的时候
            $recordsFilteredResult = AuthRule::field('id,name,title,is_menu,pid')->order($order_column, $order_dir)->limit($start,$length)->select()->toArray();
